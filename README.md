...
- `HyperGraph.py`: Defines the `HyperGraph` class (referred to as the _H3 model_ in the paper) and its extension `HyperGraphAsymmetricSI` (referred to as the _naSI model_ in the paper), an environment for a non-linear Susceptible-Infected (SI) model on hypergraphs.
