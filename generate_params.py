...
def generate_params(file_path:str='./data/', real_world:str=None, hg_file_path:str=None, real_pred_gender:bool=False,
    ...
    homophilies = ['weak', 'neutral', 'het', 'neutralweak3', 'weakneutral3', 'hetweak3', 'weakhet3', 'hetneutral3', 'neutralhet3']
    if not nus:
...
def get_edge_counts(h:str, p_m:float) -> Dict[int, Dict[int, int]]:
    ...
    if p_m == 0.5:
        if h == 'neutral':
            ...
            m = {2: {0: 3575, 1: 17850, 2: 3575}, 3: {0: 800, 1: 5200, 2: 5200, 3: 800}, 4: {0: 195, 1: 1604, 2: 2404, 3: 1604, 4: 195}}     # strength 0.5                                                                                                                             # leaving the majority where it is
        elif h=='weakneutral3':
    ...
    # imbalanced	
        ...
        if h=='neutral':
            ...
            m = {2: {0: 7200, 1: 16520, 2: 1280}, 3:{0: 2570, 1: 6976, 2: 2326, 3: 128}, 4: {0: 951, 1: 3126, 2: 1561, 3: 348, 4: 14}}     # strength 0.5                                                                                                                             # leaving the majority where it is
        elif h=='weakneutral3':
...
def run(params, file_path, hypergraphs_only=False, read_hypergraph=False, include_pairwise=False, seed_test=False, real_world=False, real_pred_gender=False):
    ...
    for i in tqdm.tqdm(range(num_hg)):
        ...
        hASI = HyperGraphAsymmetricSI(nodes=np.arange(0, num_nodes), edges=edges, group=group, nus=np.array([[nu[0],nu[1]],[nu[1],nu[0]]]), lams=np.array([[lam[0],lam[1]],[lam[1],lam[0]]]), fast=FAST, samplable_set=True)
